...
from ..datasets.coco import BODY_PARTS_KPT_IDS
import json
...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=20, min_paf_score=0.01, demo=False, defined_success_ratio=0.6, point_score=100, point_num=10):
    pose_entries = []
    ...
    for part_id in range(len(BODY_PARTS_PAF_IDS)):
        ...
        if num_kpts_a == 0 and num_kpts_b == 0:  # no keypoints for such body part
            print("no keypoints for such body part")
            continue
        ...
        for i in range(num_kpts_a):
            ...
            for j in range(num_kpts_b):
                ...
                success_ratio = 0
                if cur_point_score > -point_score:
                ...
                if success_ratio >= defined_success_ratio:
                    score_all = ratio + kpts_a[i][2] + kpts_b[j][2]
                    connections.append([i, j, abs(ratio), score_all])
        ...
        if len(connections) == 0:
            print("no connections for {}".format(part_id))
            continue
        else:
            print("found {} for {}".format(len(connections), part_id))
